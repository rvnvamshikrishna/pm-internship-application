...
def update_internship(
    ...
    internship.ctc = payload.ctc
    internship.start_date = payload.start_date
    internship.experience = payload.experience
...
def get_ranked_candidates(
    ...
    for app in applications:
        ...
        # Use AI engine for explainability reasons
        scored = score_students(internship, [student], db)
        reasons = scored[0][2] if scored else ["Meets general requirements."]
...
# -------------------------------------------------
@router.get("/{company_id}/public-stats")
def get_public_company_stats(
    company_id: int,
    db: Session = Depends(get_db)
):
    company = db.query(models.Company).filter(
    ...
    opportunities_posted = len(internships)
    
    internship_ids = [i.internship_id for i in internships]
    candidates_hired = 0
    if internship_ids:
        candidates_hired = db.query(models.Application).filter(
            models.Application.internship_id.in_(internship_ids),
            models.Application.status == models.ApplicationStatusEnum.Selected
        ).count()
        
    return {
        "hiring_since": company.created_at.strftime("%B %Y") if company.created_at else "November 2025",
